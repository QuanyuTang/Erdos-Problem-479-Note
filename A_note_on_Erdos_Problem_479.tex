...
n \mid 2^n - k.
\]
The online database OEIS contains separate entries for many of these sets. A convenient starting point is the OEIS wiki page \emph{$2^n \bmod n$}~\cite{OEIS2nmodn} and the cross-references on the page for~\href{https://oeis.org/A334634}{A334634}~\cite{A334634}. In particular, the following table of integer values of $k$ for which $A(k)$ has its own OEIS entry is adapted from the table on the OEIS wiki page~\cite{OEIS2nmodn}.
...
\bottomrule
\end{tabular}
\caption{Integer values of $k$ for which $A(k)$ currently has a dedicated OEIS entry.}
\label{tab:OEIS}
\end{table}
